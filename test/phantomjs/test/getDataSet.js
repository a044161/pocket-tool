	...
	describe('getDataSet Test', function() {
		...
		describe('Error 相关', function() {
			...
			it('参数2传入function为错误类型', function() {
				getDataSet
					.bind(getElement_string, function() {})
					.should.throw();
			});
		});
		...
		describe('获取data-set相关', function() {
			it('key的值为字符串', function() {
				getDataSet(getElement_string, DATA_target)
					.should.be.String()
					.and.equal('a');
			});

			it('key的值为数值', function() {
				getDataSet(getElement_number, DATA_target)
					.should.be.Number()
					.and.equal(1);
			});

			it('key的值为数组', function() {
				getDataSet(getElement_array, DATA_target)
					.should.be.Array()
					.and.deepEqual([1, 2, 3]);
			});

			it('key的值为对象', function() {
				getDataSet(getElement_object, DATA_target)
					.should.be.Object()
					.and.deepEqual({
						1: 'a',
						2: 'b',
